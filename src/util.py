...
#TODO this needs to be fixed, so input is less params and more flexible for \\
...
def validate_reference(author, title, booktitle, year, editor):
    ...
    fields_validation_length = {"author": author,
                                "title": title,
    ...
    for key, content in fields_validation_length.items():
        ...
        if len(content) < 5:
            raise UserInputError(
                f"Reference {key} length must be greater than 4"
            )
